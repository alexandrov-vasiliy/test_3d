...
namespace _Game.Merge
{
    public class MergeAnimator : MonoBehaviour
    {
        ...
        public void Initialize(GameAssets assets, SoundPlayer soundPlayer)
        {
            this.assets = assets;
        ...
        private void SpawnDisappearEffect([Bridge.Ref] Vector3 position, HexColor color)
        {
        ...
        private static void ApplyEffectColor(GameObject effect, [Bridge.Ref] Color color)
        {
            ParticleSystem[] particleSystems = effect.GetComponentsInChildren<ParticleSystem>(true);
            foreach (ParticleSystem particleSystem in particleSystems)
            {
